...
class SummaryView(View):
    ...
    def _time_spent_in_db_by_view(self, filters):
        ...
        for view, _ in values_list:
            r = models.Request.objects.filter(view_name=view, *filters).values('view_name').annotate(t=Sum('queries__time_taken')).order_by('-t')[0]
            requests.append(r)
        return requests
    ...
    def _num_queries_by_view(self, filters):
        ...
        for view in views:
            try:
                r = models.Request.objects.filter(view_name=view, *filters).values('view_name').annotate(t=Count('queries')).order_by('-t').first()
                requests.append(r)
            except IndexError:
                pass
        return requests
